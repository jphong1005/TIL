...
#### HDD RAID 구성
- HDD는 대용량 저장 공간을 경제적으로 구축하는데 널리 사용됨
  
- 쓰기 정책
  - Write Back
    - RAID 컨트롤러에 BBU가 장착되어 있는 경우 성능 향상을 위해 사용
    - 데이터가 먼저 컨트롤러 캐시에 기록되고 시스템에는 즉시 쓰기 완료 응답을 보낸 후에 드라이브에 기록

  - Write Through
    - BBU가 없거나 데이터 안정성을 성능보다 극도로 우선 시 하는 경우에 사용
    - 데이터가 드라이브에 완전히 기록된 후 시스템에 쓰기 완료 응답을 보냄, 안전하지만 쓰기 속도는 느림
    
- 고려사항
  - 작동 중 발열과 소음이 발생하며, 물리적 충격이나 진동에 취약하여 드라이브 고장 위험이 높음
  - GB당 비용이 SSD에 비해 저렴하여, 대용량 저장 공간을 경제적으로 구축하는데 유리
  - HDD는 시간이 지남에 따라 성능이 저하되거나 고장률이 증가할 수 있으므로, 정기적인 상태 모니터링 필요

<br>

#### SSD RAID 구성
- SSD는 뛰어난 속도, 낮은 지연 시간으로 스토리지 성능을 획적으로 향상 가능
  
- 쓰기 정책
  - Write Back 또는 Write Through 사용이 가능하며, Write Through 정책을 사용하더라도 성능 저하가 HDD만큼 발생하지 않음, 컨트롤러에 BBU가 있고 안정적인 power 공급이 보장된다면 Write Back 사용으로 추가적인 성능 향상을 꾀할 수 있음
    
- 고려사항
  - SSD의 저장매체인 NAND 플래시 메모리 셀은 쓰기 횟수에 제한이 있음
    - SSD의 내구성 또는 보통 TBW나 DWPD로 표현
  
  - 패리티를 사용하는 RAID 5와 6의 경우 '쓰기 증폭' 현상을 유발할 수 있음
    - 단일 논리적 쓰기 요청이 여러 번 물리적 쓰기 (데이터 + 패리티)를 초래하여 실제 NAND 플래시에 기록되는 양이 늘어나는 것을 의미
  
  - RAID 배열에서는 모든 드라이브가 유사한 수준의 작업이 수행되며, 미세한 사용량 차이로 동시 다발적인 장애는 발샣아지 않지만 1개의 드라이브가 수명을 다하면, 다른 드라이브들도 비슷한 상태일 수 있으니 주의하여 모니터링 해야함
